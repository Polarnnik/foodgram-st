from recipes.models import Tag, Ingredient, RecipeIngredient, Recipe,  Favorite, ShoppingCart
...
class RecipeWriteSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        tags = validated_data.pop('tags', []) # <--- Безопасно извлекаем. Если тегов нет, будет []
    ...
    def update(self, instance, validated_data):
        ...
        user = request.user
        instance.is_favorited = Favorite.objects.filter(user=user, recipe=instance).exists()
        instance.is_in_shopping_cart = ShoppingCart.objects.filter(user=user, recipe=instance).exists()
        return instance
